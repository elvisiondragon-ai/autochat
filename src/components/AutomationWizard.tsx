...
export const AutomationWizard: React.FC<AutomationWizardProps> = ({ userId, userEmail, metaInstagramId, metaPageId, metaAccessToken, initialData, onSuccess, onCancel }) => {
    ...
    // FB Posts State
    const [fbPosts, setFbPosts] = useState<IgMedia[]>([]);
    const [isLoadingFbPosts, setIsLoadingFbPosts] = useState(false);
    ...
    const fetchFbPosts = async () => {
        if (!metaPageId || !metaAccessToken) return;
        setIsLoadingFbPosts(true);
        try {
            const res = await fetch(
                `https://graph.facebook.com/v22.0/${metaPageId}/posts?fields=id,message,full_picture,created_time,permalink_url&limit=10&access_token=${metaAccessToken}`
            );
            const data = await res.json();
            if (data.error) throw new Error(data.error.message);
            const mapped: IgMedia[] = (data.data || []).map((p: any) => ({
                id: p.id,
    ...
    useEffect(() => {
        if (step === 2 && targetPost === "post_tertentu") {
            if (igPosts.length === 0 && (platformFilter === "ig" || platformFilter === "both")) fetchIgPosts();
            if (fbPosts.length === 0 && (platformFilter === "fb" || platformFilter === "both")) fetchFbPosts();
        }
    }, [step, targetPost]);
    ...
    const handleSave = async () => {
        ...
        // sequence: only validate steps that have been filled in (flexible save)
    ...
    // Save is available from step 5 onwards (not just at the final step)
    const canSave = step >= 5;
    ...
    return (
        <div className="mb-8 rounded-2xl border border-border bg-card shadow-sm overflow-hidden">
                    ...
                    <AnimatePresence mode="wait">
                        ...
                        {step === 1 && (
                            <motion.div key="step1" initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -20 }} className="space-y-4">
                                ...
                                {/* Platform Filter */}
                                {triggerSource && (
                                    <div className="mt-4 rounded-xl border border-input bg-secondary/10 p-4 space-y-3">
                                        <p className="text-sm font-medium text-foreground">Platform mana yang mau dipakai?</p>
                                        <div className="flex gap-2">
                                            {(["ig", "fb", "both"] as const).map((opt) => (
                                                <button
                                                    key={opt}
                                                    type="button"
                                                    onClick={() => setPlatformFilter(opt)}
                                                    className={`flex-1 py-2 rounded-xl border-2 text-sm font-semibold transition-all ${platformFilter === opt ? "border-primary bg-primary/10 text-primary" : "border-border text-muted-foreground hover:border-primary/50"}`}
                                                >
                                                    {opt === "ig" ? "📸 IG Only" : opt === "fb" ? "📘 FB Only" : "🔗 Both"}
                                                </button>
                                            ))}
                                        </div>
                                    </div>
                                    ...
                                    >
                                        ...
                                        {showPostPicker && (
                                            <div className="mt-3 space-y-4" onClick={e => e.stopPropagation()}>
                                                ...
                                                {/* IG Posts Section */}
                                                {(platformFilter === "ig" || platformFilter === "both") && (
                                                    <div>
                                                        <p className="text-xs font-semibold text-muted-foreground mb-2 flex items-center gap-1">📸 Instagram Posts</p>
                                                        {isLoadingPosts ? (
                                                            <div className="flex items-center justify-center py-6 gap-2 text-muted-foreground">
                                                                <Loader2 className="h-4 w-4 animate-spin" />
                                                                <span className="text-sm">Memuat IG...</span>
                                                            </div>
                                                        ) : igPosts.length === 0 ? (
                                                            <div className="text-center py-4 space-y-2">
                                                                <ImageIcon className="h-6 w-6 text-muted-foreground mx-auto" />
                                                                <p className="text-xs text-muted-foreground">Instagram belum terhubung atau tidak ada postingan.</p>
                                                                <div className="text-xs text-muted-foreground bg-muted/50 p-3 rounded-lg text-left overflow-hidden break-all font-mono space-y-1">
                                                                    <div className="font-semibold text-primary mb-1 border-b pb-1">Debug API Meta:</div>
                                                                    <div>IG ID: <span className="text-yellow-400">{metaInstagramId || "(kosong)"}</span></div>
                                                                    <div>Token: {metaAccessToken ? <span className="text-green-500">Tersimpan</span> : <span className="text-destructive">Kosong</span>}</div>
                                                                    {(!metaInstagramId || !metaAccessToken) && (
                                                                        <div className="text-amber-500 mt-1">Pastikan "Meta Terhubung" berstatus hijau.</div>
                                                                    )}
                                                                </div>
                                                                <Button size="sm" variant="outline" onClick={fetchIgPosts} className="mt-2">
                                                ...
                                                {/* FB Posts Section */}
                                                {(platformFilter === "fb" || platformFilter === "both") && (
                                                    <div>
                                                        <p className="text-xs font-semibold text-muted-foreground mb-2 flex items-center gap-1">📘 Facebook Posts</p>
                                                        {isLoadingFbPosts ? (
                                                            <div className="flex items-center justify-center py-6 gap-2 text-muted-foreground">
                                                                <Loader2 className="h-4 w-4 animate-spin" />
                                                                <span className="text-sm">Memuat FB...</span>
                                                            </div>
                                                        ) : fbPosts.length === 0 ? (
                                                            <div className="text-center py-4 space-y-2">
                                                                <ImageIcon className="h-6 w-6 text-muted-foreground mx-auto" />
                                                                <p className="text-xs text-muted-foreground">Facebook Page belum terhubung atau tidak ada postingan.</p>
                                                                <div className="text-xs text-muted-foreground bg-muted/50 p-3 rounded-lg text-left font-mono space-y-1">
                                                                    <div className="font-semibold text-primary mb-1 border-b pb-1">Debug API Meta:</div>
                                                                    <div>Page ID: <span className="text-yellow-400">{metaPageId || "(kosong)"}</span></div>
                                                                    <div>Token: {metaAccessToken ? <span className="text-green-500">Tersimpan</span> : <span className="text-destructive">Kosong</span>}</div>
                                                                </div>
                                                                <Button size="sm" variant="outline" onClick={fetchFbPosts} className="mt-2">
                                                                    <RefreshCw className="h-3 w-3 mr-1" /> Coba Lagi
                                                                </Button>
                                                            </div>
                                                        ) : (
                                                                    ...
                                                                    return (
                                                                        <div key={post.id} onClick={() => setSelectedPost(post)}
                                                                            ...
                                                                            {thumb ? (
                                                                                ...
                                                                                <div className="w-full h-full bg-[#1877f2]/20 flex items-center justify-center text-[10px] text-white/60">FB</div>
                                                                            )}
                                                                            {isSelected && (
                                                                                <div className="absolute inset-0 bg-primary/20 flex items-center justify-center">
                                                                                    <CheckCircle2 className="h-5 w-5 text-white" />
                                                                            ...
                                                                            <div className="absolute bottom-1 left-1 text-[8px] bg-[#1877f2] text-white px-1 rounded">FB</div>
                                                                        </div>
                                                                    );
                                                                })}
                                                            </div>
            ...
            <div className="border-t border-border bg-background px-6 py-4 flex items-center justify-between">
                ...
                <div className="flex items-center gap-2">
                    {canSave && (
                        <Button onClick={handleSave} disabled={isSaving} variant="outline" className="min-w-[140px] border-primary text-primary hover:bg-primary hover:text-white">
